const {getAllPetsService, getPetService, createPetService, updatePetService, deletePetService} = require('../services/mascotaService');
...
const updatePetController = async (req, res) => {
    const result = await updatePetService(req.params.objIDUsuarioMascota, req.body);
    if(result.statusCode === 200){
        res.status(200).json(result);
    }else{
        res.status(result.statusCode).json(result.mensaje)
...
const deletePetController = async (req, res) => {
    const result = await deletePetService(req.params.objIDUsuarioMascota);
    if(result.statusCode === 200){
        res.status(200).json(result);
    }else{
        res.status(result.statusCode).json(result.mensaje)
